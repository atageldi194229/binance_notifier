...
# sma volumer
multiplier = 2
v1 = talib.SMA(volumes, timeperiod=864)
v9 = v1 * multiplier
...
df["max values"] = macdhist
df["min values"] = macdhist
df["cancelled"] = macdhist
for i in range(1, len(df)):
    df.at[i, 'max values'] = max_values[-1]
    df.at[i, 'min values'] = min_values[-1]
    df.at[i, 'cancelled'] = df["cancelled"].iloc[i - 1]
    ...
    if macdhist[i - 1] > 0 and macdhist[i] <= 0:
        ...
        df.at[i, 'cancelled'] = False

    if macdhist[i] < 0 and df["ema233"].iloc[i] > df["Close"].iloc[i]:
        df.at[i, 'cancelled'] = True
...
for i in range(1, len(df)):
    ...
    cond = df["extremium"].iloc[i]
    cond = cond and df["ema8"].iloc[i] > df["ema21"].iloc[i] and df["ema21"].iloc[i] > df["ema55"].iloc[i] and df["Close"].iloc[i] > df["ema233"].iloc[i]
    cond = cond and not df["accessible"].iloc[i - 1] and df["accessible"].iloc[i]
    cond = cond and macdhist[i] > 0
    cond = cond and not df["cancelled"].iloc[i]
    ...
    if opens[i] < closes[i] and cond and len(orders) == 0: # and btc_condition: # df["accessible"].iloc[i]
        ...
        for j in range(i - 1, 2, -1):
            if not is_red and macdhist[j] <= 0:
                is_red = True
        ...
        # create_order("extremium_macd", ema21[i] if is_under_ema8 else ema8[i], "ema21" if is_under_ema8 else "ema8", closes[i], df["Close time text"].iloc[i])
        ...
        create_order(
            "extremium_macd", 
            df["min values"].iloc[i], 
            t, 
            closes[i], 
            df["Close time text"].iloc[i])
